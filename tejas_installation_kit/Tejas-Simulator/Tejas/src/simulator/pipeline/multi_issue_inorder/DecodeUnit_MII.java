...
public class DecodeUnit_MII extends SimulationElement {
	...
	public void performDecode(MultiIssueInorderPipeline inorderPipeline) {
		...
		while (ifIdLatch.isEmpty() == false && idExLatch.isFull() == false) {
			...
			if (ins != null) {
				...
				if (ins.getOperationType() == OperationType.load) {
					...
					addToValueReadyArray(ins.getSourceOperand1(),
							GlobalClock.getCurrentTime() + containingExecutionEngine.getExecutionCore().getFULatency(OpTypeToFUTypeMapping.getFUType(ins.getOperationType())));
				...
				if (ins.getOperationType() == OperationType.branch) {
					boolean prediction = containingExecutionEngine.getBranchPredictor().predict(lastValidIPSeen, ins.isBranchTaken());
					...
					containingExecutionEngine.getBranchPredictor().Train(ins.getCISCProgramCounter(), ins.isBranchTaken(), prediction);
					this.containingExecutionEngine.getBranchPredictor().incrementNumAccesses(1);
	...
	private boolean checkDataHazard(Instruction ins) {
		return !(isOperandReady(ins.getSourceOperand1()) && isOperandReady(ins.getSourceOperand2()));
	}

	private boolean isOperandReady(Operand opnd) {
		if (opnd != null) {
			if (opnd.isIntegerRegisterOperand()) {
				if (containingExecutionEngine.getValueReadyInteger()[(int) (opnd.getValue())] > GlobalClock.getCurrentTime()) {
					return false;
				}
			}

			else if (opnd.isFloatRegisterOperand()) {
				if (containingExecutionEngine.getValueReadyFloat()[(int) (opnd.getValue())] > GlobalClock.getCurrentTime()) {
					return false;
				}
			}

			else if (opnd.isMemoryOperand()) {
				return (isOperandReady(opnd.getMemoryLocationFirstOperand()) && isOperandReady(opnd.getMemoryLocationSecondOperand()));
			}
		}
	...
	private void addToValueReadyArray(Operand destOpnd, long timeWhenValueReady) {
		if (destOpnd.isIntegerRegisterOperand()) {
			containingExecutionEngine.getValueReadyInteger()[(int) (destOpnd.getValue())] = timeWhenValueReady;
		}

		else if (destOpnd.isFloatRegisterOperand()) {
			containingExecutionEngine.getValueReadyFloat()[(int) (destOpnd.getValue())] = timeWhenValueReady;
		}
	}
