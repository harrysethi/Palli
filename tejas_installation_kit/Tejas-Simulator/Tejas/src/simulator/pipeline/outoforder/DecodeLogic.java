...
public class DecodeLogic extends SimulationElement {
	...
	public void performDecode() {
		...
		for (int i = 0; i < decodeWidth; i++) {
			...
			if (headInstruction != null) {
				...
				if (headInstruction.getOperationType() == OperationType.load || headInstruction.getOperationType() == OperationType.store) {
	...
	ReorderBufferEntry makeROBEntries(Instruction newInstruction) {
		if (newInstruction != null) {
			ReorderBufferEntry newROBEntry = containingExecutionEngine.getReorderBuffer().addInstructionToROB(newInstruction, newInstruction.getThreadID());
			...
			if (newInstruction.getOperationType() == OperationType.load || newInstruction.getOperationType() == OperationType.store) {
				...
				containingExecutionEngine.getCoreMemorySystem().allocateLSQEntry(isLoad, newROBEntry.getInstruction().getSourceOperand1MemValue(), newROBEntry);
			}
