...
public class FetchLogic extends SimulationElement {
	...
	public FetchLogic(Core core, OutOrderExecutionEngine execEngine) {
		...
		instructionsToBeDropped = new OperationType[] { OperationType.interrupt, OperationType.sync };
	}
	...
	public void performFetch() {
		...
		if (!execEngine.isToStall1() && !execEngine.isToStall2() && !execEngine.isToStall3() && !execEngine.isToStall4() && !execEngine.isToStall5()) {
		...
		for (int i = 0; i < iCacheBuffer.size; i++) {
			...
			if (newInstruction.getOperationType() == OperationType.load || newInstruction.getOperationType() == OperationType.store) {
		...
		/*
		 * int noOfIterations = 0; do { inputPipeToReadNext = (inputPipeToReadNext + 1)%core.getNo_of_input_pipes(); noOfIterations++;
		 * }while(inputToPipeline[inputPipeToReadNext].isEmpty() == true && noOfIterations < fetchWidth);
		 */
	}
