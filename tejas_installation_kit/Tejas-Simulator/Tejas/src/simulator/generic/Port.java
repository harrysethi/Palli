...
public class Port {
	...
	// occupancy defines the number of clockCycles needed for a single transfer on the port.
	...
	public Port(PortType portType, int noOfPorts, long occupancy, long latencyOfConnectedElement) {
		...
		if (portType == PortType.Unlimited) {
		...
		else if (portType != PortType.Unlimited && noOfPorts > 0 && occupancy > 0) {
			// For a FCFS or a priority based port, noOfPorts and occupancy must be non-zero.
			...
			misc.Error.showErrorAndExit("Invalid initialization of port !!\n" + "port-type=" + portType + " no-of-ports=" + noOfPorts + " occupancy=" + occupancy);
		}
	}
	...
	public void put(Event event, int noOfSlots) {
		if (this.portType == PortType.Unlimited) {
			...
			// If a port is available, set its portBusyUntil field to current time
			if (portBusyUntil[availablePortID] < GlobalClock.getCurrentTime()) {
				// this port will be busy for next occupancy cycles
				portBusyUntil[availablePortID] = GlobalClock.getCurrentTime() + occupancy;
			} else {
